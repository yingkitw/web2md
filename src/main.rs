...
/// Query the terminal size as (rows, cols), falling back to 24x80.
...
fn viewport_window(total: usize, top: usize, height: usize) -> (usize, usize) {
...
fn find_match_forward(lines: &[String], pat: &str, from: usize) -> Option<usize> {
...
fn find_match_backward(lines: &[String], pat: &str, from: usize) -> Option<usize> {
    let pat = pat.to_lowercase();
    if pat.is_empty() || lines.is_empty() {
        return None;
    }
...
fn load_bookmarks(path: &std::path::Path) -> Vec<String> {
...
fn add_bookmark(path: &std::path::Path, url: &str) -> std::io::Result<()> {
